...
@interface M8ReportViewController ()<UIScrollViewDelegate,ReportProductsViewDelegate>
...
@implementation M8ReportViewController
...
-(void)initWithNavBar{
    ...
    [rightBarBtn setTitle:@"返回主页" forState:UIControlStateNormal];
...
//    reportScrollView.backgroundColor = UIColorFromRGB(0xedf0f3);
    ...
    //头部蓝色背景
    CGFloat bg_h = SCREEN_H/8;
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0,bg_h - SCREEN_H, SCREEN_W, SCREEN_H)];
    bgView.backgroundColor = LOGO_COLOR;
    [headView addSubview:bgView];
    //头像
    CGFloat img_h = 2 * SCREEN_H / 10;
    ...
    FSCustomButton *compareBtn = [[FSCustomButton alloc] initWithFrame:CGRectMake(SCREEN_W - compareBtn_w - 10,CGRectGetMaxY(bgView.frame) - compareBtn_w,compareBtn_w,compareBtn_w)];
    compareBtn.adjustsTitleTintColorAutomatically = YES;
    [compareBtn setTintColor:[UIColor whiteColor]];
    compareBtn.titleLabel.font = [UIFont boldSystemFontOfSize:16];
    ...
    _birthView =[[ImageWithStringView alloc] initWithFrame:CGRectMake(CGRectGetMinX(_headImgView.frame) - (number_w - 20), CGRectGetMaxY(_nameLabel.frame) + 5,number_w - 20, CGRectGetHeight(_nameLabel.frame)) andImage:[UIImage imageNamed:@"birthday.png"]];
    _birthView.numberLb.text = _currentCustomer.birthday;
    [headView addSubview:_birthView];
    //客户电话
    _phoneView =[[ImageWithStringView alloc] initWithFrame:CGRectMake(CGRectGetMinX(_headImgView.frame), CGRectGetMinY(_birthView.frame),CGRectGetWidth(_headImgView.frame), CGRectGetHeight(_birthView.frame)) andImage:[UIImage imageNamed:@"phone.png"]];
    _phoneView.numberLb.text = _currentCustomer.phoneNumber;
    [headView addSubview:_phoneView];
    //报告日期
    _dateView =[[ImageWithStringView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_headImgView.frame), CGRectGetMinY(_birthView.frame), number_w, CGRectGetHeight(_birthView.frame)) andImage:[UIImage imageNamed:@"birthday.png"]];
